...
## 2026-08-13 — A Foreign File Squatting on the Plan-Artifact Path
...
- Issue: the file's content belongs to an entirely different project — it discusses `AuditIssue`, `WARN-01`/`WARN-02` precedence, JSON/SARIF formatters, and a `discovery_ordinal` field, none of which exist in this codebase. It also links to `/Users/liorparente/Documents/Projects/auto-routing`, a path that is not this repository.
- Why it matters more than ordinary clutter: `skills/worker-routing/advisory_consultation.py` resolves `plan_path = root_dir / "implementation_plan.md"` and both writes and deletes exactly that path — including `_remove_stale_plan_artifact`, which exists specifically to clear "a pre-existing `implementation_plan.md` under `root_dir` from an earlier run", and which already caused the canary-deletion bug fixed in `aa118f1`. The stale file is therefore a live input to the CriticalDialogue machinery, not a dead artifact sitting beside it.
- Consequence: none observed yet — no dialogue run has been triggered against it. Reported rather than deleted, since removing it is the user's call.
- Lesson: a file whose *path* is load-bearing cannot be assessed by its content alone. Before dismissing a stray file as harmless leftovers, grep the codebase for its path — if anything reads or deletes it, it is configuration, and stale configuration is worse than none.
