...
## 2026-08-05 — Worker Routing CLI Recursion

- Issue: Nested worker CLIs executed by the Orchestrator read the protocol instructions and attempted to recursively route tasks to other workers, causing an infinite loop.
- Root Cause: Relying on the LLM to self-declare or respect nested status via loose boolean flags (`IN_WORKER_ROUTING=true`) without explicit framework interception and injection.
- Resolution: Transitioned to a definitive `ROUTING_DEPTH=1` environment variable. Created a central `constants.py` for the routing env var and a mandatory `NESTED_WORKER_WARNING` injection string. Verified strict string injection (`not in` check) via Python adapters to guarantee child workers bypass the routing gates.

## 2026-08-05 — Node 22 Parallel Test Execution Deadlock

- Issue: GitHub Actions CI process stalled (Timeout) without error output during `npm test`.
- Root Cause: Node.js 22 runs unit tests in parallel by default. The test runner spawned multiple tests simultaneously that all attempted to interact with the State Ledger's `.lock` file (create/lock/remove) in the same millisecond, creating a race condition and a subsequent deadlock where one thread locked the file and others hung infinitely.
- Resolution: Added `--test-concurrency=1` to the test script in `package.json` to force single-threaded, sequential test execution, eliminating the resource collision.
