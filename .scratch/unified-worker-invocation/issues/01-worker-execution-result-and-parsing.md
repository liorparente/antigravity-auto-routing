...
- [x] Define `WorkerExecutionResult` dataclass with fields for raw output, duration_ms, cost_estimate_usd, success, error, and parsed_payload.
- [x] Implement text & JSON extraction helper to parse vote verdicts and structured security findings from stdout without duplicating regex logic across callers.
- [x] Add unit tests verifying `WorkerExecutionResult` construction, extraction accuracy, and fail-closed error reporting.
