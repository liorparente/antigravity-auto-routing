...
- [ ] Implement collapsible bottom drawer with JetBrains Mono typography for `routing-config.json` preview.
- [ ] Dynamically regenerate the JSON payload string on every role model/effort change.
- [ ] Implement "📋 העתק קונפיגורציה" button with clipboard write and success toast.