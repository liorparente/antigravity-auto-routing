...
- [x] Wire `debate_orchestrator.py` to delegate to `debate_state_machine.py` and `debate_transport.py`.
- [x] Re-export all legacy symbols, constants, and entry points (`run_advisory_consultation_debate`, `run_critical_dialogue`).
- [x] Verify `python3 -m unittest discover -s skills/worker-routing` passes all 986+ tests with 0 errors.
- [x] Run `ruff check skills/worker-routing/` and `mypy skills/worker-routing/` with 0 warnings.
