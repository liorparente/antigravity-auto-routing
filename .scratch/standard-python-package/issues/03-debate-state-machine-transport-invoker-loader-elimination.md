...
- [ ] Replace `_load_sibling` in state machine, transport, transcript, and invoker with standard relative imports.
- [ ] Verify mock monkeypatching in `test_debate_transport.py` and `test_production_invoker.py` intercepts calls seamlessly without module identity splitting.
- [ ] Verify `test_debate_state_machine.py` and `test_dialogue_transcript.py` pass cleanly.
