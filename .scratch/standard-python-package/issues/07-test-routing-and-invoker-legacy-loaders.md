...
**What to build:** Eliminate the remaining by-path `importlib.util.spec_from_file_location` loaders in
`test_routing.py` (lines 72, 78, 86, 94, 1400, 5568, 7065, 7367) and `test_production_invoker.py`
(lines 383, 452), replacing them with standard hybrid package/standalone imports of the now-package
modules they load dynamically (`routing_check`, `agent_council`, `advisory_consultation`,
`learned_state`, `learning_journal`, `learning_outcomes`, `production_invoker`).

**Blocked by:** none — spec 0010's sibling-loader elimination (Tickets 01–06) is otherwise complete;
these two test files are the last holdouts.
