...
- [ ] Replace all `_load_sibling` calls in `debate_orchestrator.py` with clean relative imports.
- [ ] Replace dynamic `_modules` and `__getattr__` in `advisory_consultation.py` with explicit static imports.
- [ ] Verify `test_debate_orchestrator.py` and `skills/council-review/tests/test_council_review.py` pass with zero regression.
