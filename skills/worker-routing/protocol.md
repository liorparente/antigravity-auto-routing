...
| **Medium** | 3-4 files, new feature, refactoring | **Claude Sonnet 5 (Thinking)** / **Gemini 3.6 Flash (High)** (+ Codex Sol Critic) | `IN_WORKER_ROUTING=true claude -p --no-session-persistence --model claude-sonnet-5 -c model_reasoning_effort="high" --allow-dangerously-skip-permissions "..." < /dev/null` <br> Critic effort: `medium` / `high` |
