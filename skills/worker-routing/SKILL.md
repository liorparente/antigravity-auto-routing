...
* **Command:** `ROUTING_DEPTH=1 script -q /dev/null agy -p "Perform deep research on {TOPIC}. Map out all affected files, imports, exported interfaces, and potential breaking changes."`
...
2. **Autonomous Debate Loop:** The **Critic** (Codex 5.6 Sol / GPT-OSS 120B) reviews the draft plan using `medium`/`high` reasoning effort, flagging missing edge cases or performance flaws. Up to 3 rounds until consensus.
   * **Command:** `cat .claude/plan_draft.md | ROUTING_DEPTH=1 codex exec --model gpt-5.6-sol -c model_reasoning_effort="high" "Perform deep review of this plan. Check for race conditions, type safety, edge cases, and performance." < /dev/null`
...
* The Orchestrator invokes **Codex 5.6 Sol** with `high` effort for a final audit of the diff:
  * **Command:** `ROUTING_DEPTH=1 codex review --uncommitted -s workspace-write -c model="gpt-5.6-sol" -c model_reasoning_effort="high" < /dev/null`
